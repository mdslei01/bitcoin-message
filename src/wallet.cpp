...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, bool useTransactionFee)
{
    int64 nValue = 0;
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                if (useTransactionFee && nFeeRet < MIN_TX_FEE && nChange > 0 && nChange < CENT)
                {
                ...
                bool fAllowFree = !useTransactionFee || CTransaction::AllowFree(dPriority);
                int64 nMinFee = wtxNew.GetMinFee(1, fAllowFree, GMF_SEND);
                if (useTransactionFee && nFeeRet < max(nPayFee, nMinFee))
                {
